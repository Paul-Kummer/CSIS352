...
namespace modArray
{
	...
	//Destructor
	//precondition: none
	//postcondition: list is cleared and count set to 0
	template<class T>
	ModArray<T>::~ModArray()
	{
		this->clear();
	...
	T& ModArray<T>::operator[](int indexNum)
	{
		if(!this->empty())
		{
			...
			if(indexNum < 0)
			{
				...
				typename std::list<T>::iterator it = this->internalList.begin();
	...
	//precondition: elements in the array need an output method
	//postcondition: list of elemets is outputted
	template<class T>
	...
	//erases elements from the list and sets count to zero
	//precondition: ModArray is instantiated and elements have ways of being destroyed
	//postcondition: interlist is cleared and count is zero
	template<class T>
	void ModArray<T>::clear()
	{
		this->internalList.clear();
		this->count = 0;
	}
